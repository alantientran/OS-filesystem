...
bool dir_remove (struct dir *dir, const char *name)
{
  ...
  // Jeanie driving
  /* Check if directory is empty or in use*/
  if (inode_dir(inode)) {
    if (get_num_open(inode) > 1) {
      goto done;
    }
    ...
    while (inode_read_at (inode, &dir_e, sizeof dir_e, pos) == sizeof dir_e) {
      pos += sizeof dir_e;
      if (dir_e.in_use) {
        goto done;
      }
...
// Cecilia driving
/* Checks if dir is the root directory */
bool is_root (struct dir* dir)
{
  return dir && inode_get_inumber(dir_get_inode(dir)) == ROOT_DIR_SECTOR;
} 

/* Returns parent directory's inode from dir*/
